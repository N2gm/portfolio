...
export default function GlowCard({
  ...
  const resolvedWidth = width
    ...
    : !customSize ? sizeMap[size].width : undefined;
  const resolvedHeight = height
    ? typeof height === "number" ? `${height}px` : height
    : !customSize ? sizeMap[size].height : undefined;

  const inlineStyle: CSSProperties & Record<string, unknown> = {
    "--base": GOLD_BASE,
    "--spread": GOLD_SPREAD,
    "--radius": "14",
    "--border": "2",
    ...
    backgroundImage: `radial-gradient(var(--spotlight-size) var(--spotlight-size) at calc(var(--x, 0) * 1px) calc(var(--y, 0) * 1px), hsl(var(--hue, 43) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 70) * 1%) / var(--bg-spot-opacity, 0.08)), transparent)`,
  ...
  return (
    <>
      ...
      <div ref={cardRef} data-glow style={inlineStyle as CSSProperties} className={className}>
        <div data-glow />
        {children}
      </div>
    </>
